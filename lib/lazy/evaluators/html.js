...
module.exports = {
  ...
  getAssets: function(asset) {
    ...
    if (isBower) {
      if (!utils.bower.exists(href)) {
        utils.bower.install(href, asset.version);
      }
  ...
  evaluate: function(res, url, content, options) {
    //
    // HTML file not found?
    // Forward it to respective template compiler.
    //
    // TODO: create an evaluator forwarder to use here
    //
    if (content == null) {
      var templatesFound = glob.sync(url.replace(/\.html$/, ".{hbs,handlebars}").substr(1), {
        cwd: options.path
      });

      if (templatesFound.length > 0) {
        content = fs.readFileSync(options.path + "/" + templatesFound[0], { encoding: 'utf-8' })
        return handlebars.evaluate(res, templatesFound[0], content, options);
      }
    }
    ...
    // output websocket asset watcher on development mode.
    if (!options.optimize) {
      assetsReplacement += watcher.getClientScript() + "\n";
    }
