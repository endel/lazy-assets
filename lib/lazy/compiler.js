...
module.exports = {
  htmlTemplates: ['hbs', 'handlebars'],

  compile: function(input, output) {
    var htmls = glob.sync(input + "/**/**/**/**.{html," + this.htmlTemplates.join(",") + "}"), // , {cwd: '.'}
    ...
    for (var i=0; i < htmls.length; i++) {
      ...
      var javascripts = "", stylesheets = "",
          outputFile = htmls[i].replace(input, output).replace(new RegExp(".(" + this.htmlTemplates.join("|") + ")$"), ".html"),
      ...
      for (var y=0; y < assetFiles.length; y++) {
        ...
        var response = evaluator.evaluate(fakeRequest, assetFiles[y], content, {
